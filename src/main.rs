...
fn main() {
    //每个正常的学校都会有很多位评委, 这是预选评委(并不是每个人都能参评),
    //要求评委数量小于学校数量, 我们就默认每个学校最多有1个评委, 最少有0个评委,
    //就是从这些预选评委中筛掉一些人, 用一个0到1的随机浮点筛掉一些评委(总评委数 * 这个随机浮点) 强制转化成整数, 作为真正能参评的评委数.
    //用rand库洗牌所有学校, 洗牌所有论文, 洗牌所有评委,
    //最后将洗牌过的再随机地插入BTreeMap里, key是论文编号(每篇论文独一无二), val是评委名字(也是独一无二),
