...
impl Pattern {
    pub fn processor(&mut self) {
        ...
        //判断前两个字母--学校名字, 如果论文前两个字母和评委前两个字母完全一样, 就认为本校评委批改了本校论文,
        //这显然违反了回避原则, 所以如果相等的话, 就什么都不做, 此次外部循环重新开始, 不消耗次数, 内部循环是死循环, 无需担心次数问题;
        //不相等, 就认为没违反回避原则, 就插入此论文(key)和此评委(value)到BTreeMap, 并用remove方法销毁此论文, 此次外部循环结束, 消耗次数,
        //依次迭代......, 直到所有论文和评委被插入BTreeMap里, 且完全遵守回避原则.
